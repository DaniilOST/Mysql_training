...
select currency, count(client_id) as count
from accounts
group by currency;

select currency, round(avg(client_id)) as round_count
from accounts
group by currency;

select currency, min(client_id) as min_count, max(client_id) as max_count
from accounts
group by currency;

select currency, sum(client_id) as sum_count
from accounts
group by currency;

select currency, truncate(avg(client_id),2) as round_count
from accounts
group by currency;
...
SELECT
    ...
    CASE
        WHEN account_type_id = 5 THEN 'rub'
        ELSE 'not rub'
