...
#1) найти всех пользователей с картами у которых срок действия заканчивается в этом году

SELECT U.username, C.card_number, C.expiry_date
FROM Users as U
JOIN CreditCards as C ON U.user_id = C.user_id
WHERE YEAR(C.expiry_date) = YEAR(CURRENT_DATE);

#2) найти все транзакции у определенного пользователя(user_id = 1) за сегодня
...
#3) найти всех пользователей у которых две буквы "б" в почте и на счету у которых больше 1000

SELECT U.username
FROM Users as U
JOIN Accounts as A ON U.user_id = A.user_id
WHERE U.email LIKE '%b%b%' AND A.balance > 1000;
