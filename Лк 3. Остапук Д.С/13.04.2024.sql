...
-- Выборка всех данных из таблицы clients
SELECT * FROM clients;

-- Выборка данных из таблицы clients, где фамилия равна 'Иванов'
SELECT * FROM clients WHERE last_name = 'Иванов';

-- Запрос с группировкой количества счетов по их типу
SELECT account_type_id, COUNT(*) as count_accounts FROM accounts GROUP BY account_type_id;

-- Запрос с группировкой количества счетов по их типу и фильтрацией по количеству (больше одного счета)
SELECT account_type_id, COUNT(*) as count_accounts FROM accounts GROUP BY account_type_id HAVING count_accounts > 1;

-- Запрос на выборку всех данных из таблицы clients, отсортированных по фамилии в алфавитном порядке
